...
BEGIN
    ...
    SET @stmt12 = CONCAT("SET @OLD_SUPPLY_BONDS_BASE = (SELECT SUPPLY_OF_BONDS FROM ", @BASE_COUNTRY, " WHERE CURRENCY_ISO = '", @BASE, "')");
    PREPARE stmt12 FROM @stmt12;
    EXECUTE stmt12;
    DEALLOCATE PREPARE stmt12;
    IF @OLD_SUPPLY_BONDS_BASE < 0 THEN
    SET @NEW_SUPPLY_BONDS_BASE = RAND() * 10000000;

    SET @stmt13 = CONCAT("UPDATE ", @BASE_COUNTRY, " SET SUPPLY_OF_BONDS = ", @NEW_SUPPLY_BONDS_BASE, " WHERE CURRENCY_ISO = '", @BASE, "'");
    PREPARE stmt13 FROM @stmt13;
    EXECUTE stmt13;
