DELIMITER $$
DROP PROCEDURE IF EXISTS buy_bond_cny_usd $$
CREATE PROCEDURE buy_bond_cny_usd()
BEGIN
    SET @RESERVE_MONEY = (SELECT SUPPLY_OF_CURRENCY FROM USA WHERE CURRENCY_ISO = 'CNY');
    SET @PERCENTAGE_SPENT = .01;
    SET @AMT_SPENT = @RESERVE_MONEY * @PERCENTAGE_SPENT;   

    IF @RESERVE_MONEY > 0 THEN
        call bonds_aux_cny_usd(@RESERVE_MONEY, @AMT_SPENT);
    ELSE
    SET @AMT_SPENT = RAND() * 1000;

        WHILE @RESERVE_MONEY <= @AMT_SPENT DO
        call generate_usd_trader(2, 'USD', 'CNY', 1);
        call marketplace('CNY', 'USD');
