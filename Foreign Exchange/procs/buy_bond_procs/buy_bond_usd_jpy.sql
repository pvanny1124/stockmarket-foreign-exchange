DELIMITER $$
DROP PROCEDURE IF EXISTS buy_bond_usd_jpy $$
CREATE PROCEDURE buy_bond_usd_jpy()
BEGIN
    SET @RESERVE_MONEY = (SELECT SUPPLY_OF_CURRENCY FROM JAPAN WHERE CURRENCY_ISO = 'USD');
    SET @PERCENTAGE_SPENT = .01;
    SET @AMT_SPENT = @RESERVE_MONEY * @PERCENTAGE_SPENT;   

    IF @RESERVE_MONEY > 0 THEN
        call bonds_aux_usd_jpy(@RESERVE_MONEY, @AMT_SPENT);
    ELSE
    SET @AMT_SPENT = RAND() * 1000;

        WHILE @RESERVE_MONEY <= @AMT_SPENT DO
        call generate_jpy_trader(2, 'JPY', 'USD', 1);
        call marketplace('USD', 'JPY');
