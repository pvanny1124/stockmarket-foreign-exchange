DELIMITER $$
DROP PROCEDURE IF EXISTS buy_bond_usd_cny $$
CREATE PROCEDURE buy_bond_usd_cny()
BEGIN
    SET @RESERVE_MONEY = (SELECT SUPPLY_OF_CURRENCY FROM CHINA WHERE CURRENCY_ISO = 'USD');
    SET @PERCENTAGE_SPENT = .01;
    SET @AMT_SPENT = @RESERVE_MONEY * @PERCENTAGE_SPENT;   

    IF @RESERVE_MONEY > 0 THEN
        call bonds_aux_usd_cny(@RESERVE_MONEY, @AMT_SPENT);
    ELSE
    SET @AMT_SPENT = RAND() * 1000;

        WHILE @RESERVE_MONEY <= @AMT_SPENT DO
        call generate_cny_trader(2, 'CNY', 'USD', 1);
        call marketplace('USD', 'CNY');
