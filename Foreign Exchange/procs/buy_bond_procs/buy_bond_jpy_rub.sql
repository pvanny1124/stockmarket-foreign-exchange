DELIMITER $$
DROP PROCEDURE IF EXISTS buy_bond_jpy_rub $$
CREATE PROCEDURE buy_bond_jpy_rub()
BEGIN
    SET @RESERVE_MONEY = (SELECT SUPPLY_OF_CURRENCY FROM RUSSIA WHERE CURRENCY_ISO = 'JPY');
    SET @PERCENTAGE_SPENT = .01;
    SET @AMT_SPENT = @RESERVE_MONEY * @PERCENTAGE_SPENT;   

    IF @RESERVE_MONEY > 0 THEN
        call bonds_aux_jpy_rub(@RESERVE_MONEY, @AMT_SPENT);
    ELSE
    SET @AMT_SPENT = RAND() * 1000;

        WHILE @RESERVE_MONEY <= @AMT_SPENT DO
        call generate_rub_trader(2, 'RUB', 'JPY', 1);
        call marketplace('JPY', 'RUB');
