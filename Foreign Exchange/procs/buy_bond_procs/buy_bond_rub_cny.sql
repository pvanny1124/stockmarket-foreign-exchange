DELIMITER $$
DROP PROCEDURE IF EXISTS buy_bond_rub_cny $$
CREATE PROCEDURE buy_bond_rub_cny()
BEGIN
    SET @RESERVE_MONEY = (SELECT SUPPLY_OF_CURRENCY FROM CHINA WHERE CURRENCY_ISO = 'RUB');
    SET @PERCENTAGE_SPENT = .01;
    SET @AMT_SPENT = @RESERVE_MONEY * @PERCENTAGE_SPENT;   

    IF @RESERVE_MONEY > 0 THEN
        call bonds_aux_rub_cny(@RESERVE_MONEY, @AMT_SPENT);
    ELSE
    SET @AMT_SPENT = RAND() * 1000;

        WHILE @RESERVE_MONEY <= @AMT_SPENT DO
        call generate_cny_trader(2, 'CNY', 'RUB', 1);
        call marketplace('RUB', 'CNY');
