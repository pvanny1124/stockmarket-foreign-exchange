DELIMITER $$
DROP PROCEDURE IF EXISTS bonds_aux_jpy_rub $$
CREATE PROCEDURE bonds_aux_jpy_rub(IN RESERVE_MONEY DECIMAL(18, 2), IN AMT_SPENT DECIMAL(18, 2))
    BEGIN 
        ...
        SET @OLD_SUPPLY_CURR_TRADE_WITH = (SELECT SUPPLY_OF_CURRENCY FROM RUSSIA WHERE CURRENCY_ISO = 'JPY');
            
        SET @NEW_SUPPLY_CURR_TRADE_WITH =  @OLD_SUPPLY_CURR_TRADE_WITH - @AMT_SPENT;

        UPDATE RUSSIA SET SUPPLY_OF_CURRENCY = @NEW_SUPPLY_CURR_TRADE_WITH WHERE CURRENCY_ISO = 'JPY';
    
        SET @OLD_SUPPLY_BONDS = (SELECT SUPPLY_OF_BONDS FROM RUSSIA WHERE CURRENCY_ISO = 'JPY');
        ...
        UPDATE RUSSIA SET SUPPLY_OF_BONDS = @NEW_SUPPLY_BONDS WHERE CURRENCY_ISO = 'JPY';
