...
CREATE TRIGGER change_interest_rate 
	...
	BEGIN
		IF NEW.USD < (OLD.USD * 0.9) THEN
			SET @OLD_BOND_COUPON = (SELECT COUPON FROM BOND_RATES WHERE COUNTRY_ISO = 'USD');
		...
		ELSEIF NEW.CNY < (OLD.CNY * 0.9) THEN
			SET @OLD_BOND_COUPON = (SELECT COUPON FROM BOND_RATES WHERE COUNTRY_ISO = 'CNY');
			UPDATE BOND_RATES SET COUPON = @OLD_BOND_COUPON * 1.1;	
		...
		ELSEIF NEW.RUB < (OLD.RUB * 0.9) THEN
			SET @OLD_BOND_COUPON = (SELECT COUPON FROM BOND_RATES WHERE COUNTRY_ISO = 'RUB');
			UPDATE BOND_RATES SET COUPON = @OLD_BOND_COUPON * 1.1;	
		...
		ELSEIF NEW.JPY < (OLD.JPY * 0.9) THEN
			SET @OLD_BOND_COUPON = (SELECT COUPON FROM BOND_RATES WHERE COUNTRY_ISO = 'JPY');
			UPDATE BOND_RATES SET COUPON = @OLD_BOND_COUPON * 1.1;	
		...
		ELSEIF NEW.EUR < (OLD.EUR * 0.9) THEN
			SET @OLD_BOND_COUPON = (SELECT COUPON FROM BOND_RATES WHERE COUNTRY_ISO = 'EUR');
			UPDATE BOND_RATES SET COUPON = @OLD_BOND_COUPON * 1.1;	
